(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["fluxibleLogger"] = factory();
	else
		root["fluxibleLogger"] = factory();
})(this, function() {
	  ...
	  return function (actionContext) {
	    return function (next) {
	      return function (type, payload) {
	        ...
	        logEntry.action = { type: type, payload: payload };
...
/***/ function(module, exports) {
	...
	function getStoreNamesByActionType(stores, type) {
	  return stores.map(function (store) {
	    return { types: Object.keys(store.handlers), name: store.storeName };
	  }).filter(function (store) {
	    return store.types.filter(function (ty) {
	      return ty === type;
	    }).length > 0;
	  }).map(function (store) {
	    return store.name;
	  }).filter(function (value, index, el) {
	    return el.indexOf(value) === index;
	  });
	}

	function getStates() {
	  var storeNames = arguments.length <= 0 || arguments[0] === undefined ? [] : arguments[0];
	  var actionContext = arguments[1];

	  return storeNames.reduce(function (acc, name) {
	    return _extends({}, acc, _defineProperty({}, name, actionContext.getStore(name).state));
	  }, {});
	}
	...
	exports.default = {
	  ...
	  duration: true,
	  timestamp: true,
	  stateTransformer: function stateTransformer(stores, type, actionContext) {
	    return getStates(getStoreNamesByActionType(stores, type), actionContext);
	  },
